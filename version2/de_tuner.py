...
class DE(object):
    ...
    :param goal: tuning goal, can be "PD, PF, F, PREC, G, ACC" ect
...
class DE_Tune_ML(DE):
    ...
    def get_target_score(self, score_dict):
        temp = {}
        if self.target_class in score_dict.keys():
            temp[self.target_class] = score_dict[self.target_class][0]
        else:
            temp[self.target_class] = score_dict["mean_weighted"]
        return temp
